  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 py-4 transition-all duration-300 ${
        isScrolled
          ? "bg-engineer-dark/95 backdrop-blur-sm shadow-md"
          : "bg-transparent"
      }`}
    >
      <div className="container mx-auto flex items-center justify-between container-padding">
        <a href="#" className="flex items-center gap-2">
          ...
          <span className="text-white font-medium text-lg hidden sm:inline-block">MechEngineer</span>
        </a>
        ...
        <div className="hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-white hover:text-engineer-light transition-colors relative group"
            >
              {link.name}
        ...
        <div className="container mx-auto container-padding py-4 bg-engineer-surface/90 backdrop-blur-sm flex flex-col gap-4">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-white hover:text-engineer-light transition-colors py-2"
